...
def sfb2d_atrous_mc(LL, LH, HL, HH, filts, mode='symmetric'):

    g0_col, g1_col, g0_row, g1_row = filts
    lo = sfb1d_atrous_mc(LL, LH, g0_col, g1_col, mode=mode, dim=2)
    hi = sfb1d_atrous_mc(HL, HH, g0_col, g1_col, mode=mode, dim=2)

    y = sfb1d_atrous_mc(lo, hi, g0_row, g1_row, mode=mode, dim=3)
    return y

def sfb1d_atrous_mc(lo, hi, g0, g1, mode='symmetric', dim=-1, dilation=1):

    C = lo.shape[1]
    d = dim % 4

    if not isinstance(g0, torch.Tensor):
        g0 = torch.tensor(np.array(g0).ravel(), dtype=lo.dtype, device=lo.device)
    if not isinstance(g1, torch.Tensor):
        g1 = torch.tensor(np.array(g1).ravel(), dtype=lo.dtype, device=lo.device)
    ...
    g0 = g0.repeat(C, 1, 1, 1)
    g1 = g1.repeat(C, 1, 1, 1)
    ...
    out_lo = F.conv_transpose2d(lo, g0, padding=unpad, groups=C, dilation=dilation)
    out_hi = F.conv_transpose2d(hi, g1, padding=unpad, groups=C, dilation=dilation)
    y = out_lo + out_hi
    y = y / (2 * dilation)
    return y

def mswt_inverse_single_level(x_4c, filts, mode='symmetric'):
    B, totalC, H, W = x_4c.shape
    C = totalC // 4
    ...
    y = sfb2d_atrous_mc(LL, LH, HL, HH, filts, mode=mode)
    return y

class SWTInverseMultiChannel(nn.Module):
    def __init__(self, wave='db1', mode='symmetric'):
        super().__init__()
        if isinstance(wave, str):
            wave = pywt.Wavelet(wave)
        if isinstance(wave, pywt.Wavelet):
            ...
            raise ValueError("wave not recognized")
    ...
    def forward(self, x_4c):
        filts = (self.g0_col, self.g1_col, self.g0_row, self.g1_row)
        return mswt_inverse_single_level(x_4c, filts, mode=self.mode)
...
if __name__ == '__main__':
    ...
    xx = torch.tensor(img3).float().cuda()
    ...
    g0_col, g1_col = pywt.Wavelet(wave).rec_lo, pywt.Wavelet(wave).rec_hi
    filts = prep_filt_sfb2d(g0_col, g1_col, g0_col, g1_col, device=xx.device)

    x_rec = mswt_inverse_single_level(x_4c, filts, mode)
    print(f'Input shape: {xx.shape}, Reconstructed shape: {x_rec.shape}')